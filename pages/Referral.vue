<template>
  <div class="px-16">
      ...
      <v-col cols="6">
        <v-card width="100%" class="px-12 py-6">
          ...
          <div v-if="defaultCode.link">
            <a-row class="align-center">
              <p class="text-h4 mb-0">{{ defaultCode.code }}</p>
            ...
            <a-row class="align-center">
              <p class="mb-0" style="font-family: sans-serif !important;">{{ defaultCode.link }}</p>
    ...
    <v-row>
      <v-col cols="4">
        <stats-card title="سود دریافتی از دعوت دوستان" icon="mdi-finance" icon-color="green"
                    :body="totalProfit"/>
      </v-col>
      <v-col cols="4">
        <stats-card title="تعداد کل دوستان" icon="mdi-account-multiple" icon-color="pink"
                    :body="totalMembers"/>
      </v-col>
      <v-col cols="4">
        <stats-card title="تعداد معاملات دوستان" icon="mdi-clipboard-list" icon-color="yellow"
                    :body="totalTrades"/>
      </v-col>
      <!--      <v-col cols="3">-->
    ...
    <p class="text-center text-h6 mt-16 mb-n1">لیست لینک های دعوت</p>
    <div class="text-left">
    ...
    <v-data-table :headers="codes.headers" :items="codes.items" hide-default-footer>
    ...
    <p class="text-center text-h6 mb-8 mt-16">لیست دوستان</p>
    <v-data-table :headers="members.headers" :items="members.items" hide-default-footer
                  class="mb-12">
    ...
    <v-dialog v-model="d.create" width="520px">
      <v-card class="px-12 py-6 text-center">
        ...
        <div class="text-left">
          <v-btn @click="createCode" :loading="l.create"
...
export default {
  ...
  computed: {
    ...
    totalTrades() {
      return collect(this.members.items)
          .reduce((c, i) => c + i.trade_count, 0)
    },
    totalMembers() {
    ...
    return {
      ...
      codes: {
        headers: [
          ...
          {value: 'percents', text: 'درصد دریافتی شما/دریافتی دوستان', align: 'center', sortable: false},
          {value: 'desc', text: 'متن', align: 'center', sortable: false},
          {value: 'guest_count', text: 'دوستان', align: 'center', sortable: false},
      ...
      members: {
        headers: [
          ...
          {text: 'زمان ثبت نام', value: 'user_created_at', align: 'center', sortable: false},
          {text: 'سود دریافتی شما', value: 'profit', align: 'center', sortable: false},
          {text: 'تعداد معاملات', value: 'trade_count', align: 'center', sortable: false},
        ],
        items: []
      }
    }
  ...
  mounted() {
    ...
    this.$axios.$get('/referrals/me/my-host')
        .then(res => this.myHostCode = res)

    this.$axios.$get('/referrals/me/members')
  ...
  methods: {
    async refreshCodesList() {
      ...
      this.defaultCode.link = items.base_url + '/Register?referral_code=' + lastItem.code
