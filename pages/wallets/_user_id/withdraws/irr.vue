<template>
  <div class="mt-0">
    ...
    >
      <ul>
        کاربر گرامی، با تایید شما، برداشت با توجه به مقرات سیکل پرداخت بانکی
        انجام خواهد شد.
      </ul>
    </v-alert>
    ...
    <a-row class="align-stretch">
      <v-col cols="4" class="pa-4 d-flex flex-column">
        <div class="text-h4 primary--text mb-6 text-right">برداشت</div>
...
export default {
  ...
  computed: {
    ...
    actualAmount() {
      let amount = safeDecimal(this.amount - this.withdrawFee).todp(
        getDp(this.type)
      )
  ...
  mounted() {
    ...
    this.getFee()
    this.$axios
      .$post("/access/limits/remained/" + this.user_id, { resource: "irr" })
      .then((res) => {
  ...
  methods: {
    ...
    async withdraw() {
      try {
        ...
        await this.$axios.$post("/irr/withdraw/" + this.user_id, {
          amount: this.amount,
          shaba: this.shaba,
        })
    ...
    getFee() {
      this.$axios
        .$get("/crypto/fees/" + this.type.toLowerCase())
