<template>
  <div>
    <p class="text-h4 primary--text">بنرها و پیام های پیشواز</p>
    <a-paged-table
      ref="table"
    ...
    >
      <template v-slot:item.delete="{ item }">
        <v-btn :loading="l.delete" color="error" @click="remove(item.id)"
          >حذف</v-btn
        >
      </template>
    ...
    <v-dialog v-model="d.create" width="520px">
      <v-card class="px-12 py-6 text-center">
        <p class="text-h4 primary--text mb-8 primary--text text-right">
          ایجاد بنر
        </p>
...
export default {
  ...
  computed: {
    ...
    typeItems: () => [
      "banner_dashboard_green",
      "banner_dashboard_normal",
      "banner_dashboard_red",
      "banner_wallets_green",
      "banner_wallets_normal",
      "banner_wallets_red",
