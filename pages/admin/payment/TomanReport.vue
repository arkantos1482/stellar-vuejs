<template>
  <div class="container">
    ...
    >
      ...
      <template v-slot:item.verify_status="{ value }">{{
        value | toFa
      }}</template>
      <template v-slot:item.pay_system_status="{ value }">{{
        value | toFa
      }}</template>
      <template v-slot:item.stellar_status="{ value }">{{
        value | toFa
      }}</template>

      <template v-slot:item.verify_updated_at="{ value }">{{
        value | toFarsiDate
      }}</template>
      <template v-slot:item.stellar_updated_at="{ value }">{{
        value | toFarsiDate
      }}</template>
      ...
      <template v-slot:item.correction="{ item }">
        <span v-if="alreadyCorrected(item)" class="success--text"
          >اصلاح شده</span
        >
        <v-btn
          v-else-if="needCorrection(item)"
          outlined
          color="error"
          small
          @click="correctDialog(item)"
          >تصحیح</v-btn
        >
      </template>
    </a-paged-table>
  ...
  data() {
    return {
      ...
      filterQuery: [
        ...
        {
          type: "time",
          key: "verify_updated_at",
          name: "تاریخ وریفای",
          value: "",
        },
        {
          type: "time",
          key: "stellar_updated_at",
          name: "تاریخ استلار",
          value: "",
        },
      ],
  ...
  methods: {
    needCorrection: (item) => {
      if (item?.type === "deposit") {
        if (
          ["SUCCESSFUL", "ALREADY_VERIFIED"].includes(item.verify_status) &&
